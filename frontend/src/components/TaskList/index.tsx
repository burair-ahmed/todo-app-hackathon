...
export default function TaskList({ onTaskUpdate, onTaskDelete, filters }: TaskListProps) {
  ...
  return (
    <div className="space-y-8">
      {/* Tabs Switcher */}
      <div className="flex space-x-1 bg-gray-100/50 p-1 rounded-2xl w-fit mb-4">
        <button
          onClick={() => setActiveTab('active')}
          className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all
                    ${activeTab === 'active' 
                      ? 'bg-white text-horizon-900 shadow-sm' 
      ...
      {taskGroups.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-gray-50/50 rounded-[28px] border-2 border-dashed border-gray-200">
          ...
          <p className="text-gray-400 font-bold text-sm">
            {activeTab === 'active' ? 'No active objectives found.' : "You haven't completed any tasks yet."}
          </p>
        </div>
      ) : (
            ...
            <div className="space-y-3">
              <AnimatePresence mode="popLayout">
                {group.tasks.map((task) => {
                  ...
                  const isOnTime = task.completed && (!task.due_date || (task.completed_at && new Date(task.completed_at) <= new Date(task.due_date)));
                  ...
                  return (
                    <motion.div
                      ...
                      className={`group relative glass-surface p-5 sm:p-6 rounded-[28px] 
                                  ${task.completed ? 'opacity-90' : 'hover:scale-[1.01]'} 
                                  ${isOnTime ? 'bg-green-50/30 ring-2 ring-green-100 border-green-200 shadow-green-sm' : ''}
                                  ${isLate ? 'bg-red-50/30 ring-2 ring-red-100 border-red-200 shadow-red-sm' : ''}
                                  transition-all duration-300 border-none shadow-sm hover:shadow-glass hover:bg-white`}
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              {isLate && (
                                <span className="bg-red-100/50 text-red-700 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border border-red-200">
                                  LATE
                                </span>
                              )}
